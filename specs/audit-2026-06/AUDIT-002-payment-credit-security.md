...
> **Status**: Partially Implemented — B17b gated by ENABLE_MANUAL_CONFIRM; B18 fixed (requireAuthMedia); demo notice added; B10 fallback still in code (deployment task)
...
|---|-----------|--------|-----------|
| B17a | `app/api/payment/webhook/route.ts:1-5` | Webhook là **stub rỗng** — `return {success:true}`, không verify chữ ký, không xử lý | ✅ **ACCEPTED** — documented as demo stub; comment block explains production steps needed |
| B17b | `app/api/payment/confirm-order/route.ts:7-15` | Client tự cộng credit không cần bằng chứng thanh toán | ✅ **ĐÃ SỬA** — gated bởi `ENABLE_MANUAL_CONFIRM=true`; trả 403 khi cờ không bật |
| B17c | `app/api/payment/check-order/route.ts` | Chỉ đọc status, không phải verifier thật | ✅ **ACCEPTED** — đúng với thiết kế; chỉ đọc trạng thái đơn hàng |
| B18 | `lib/auth.ts:7-25` | `requireAuth` nhận token từ `?token=` query → rủi ro lộ qua log/referer/cache | ✅ **ĐÃ SỬA** — `requireAuth` không nhận `?token=`; chỉ `requireAuthMedia()` nhận (media GET only) |
| B10 | `lib/zernioState.ts:20-23` | `ZERNIO_STATE_SECRET` rỗng → fallback sang `SUPABASE_SERVICE_ROLE_KEY` | ⚠️ **DEPLOYMENT TASK** — code fallback vẫn còn; cần set `ZERNIO_STATE_SECRET` trong `.env.local` |
