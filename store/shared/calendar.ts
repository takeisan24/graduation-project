...
interface ScheduledCalendarPost {
  id: string;
  platform: string;
  scheduled_at: string;
  status: string;
  post_url?: string | null;
  payload?: ScheduledCalendarPostPayload;
  content_drafts?: {
...
interface CalendarStatusUpdate {
  noteType: CalendarEventType;
  status: LateLifecycleStatus;
  url?: string | null;
  isSyncFromLoad?: boolean;
}
...
function mapScheduledPostToCalendarEvent(post: ScheduledCalendarPost): { dateKey: string; event: CalendarEvent } | null {
  if (!post?.id || !post?.scheduled_at) {
    return null;
  }
  ...
  const content =
    post.content_drafts?.text_content?.trim() ||
    String(post.payload?.text_content || post.payload?.text || '').trim() ||
    undefined;
  const mediaUrls =
    post.content_drafts?.media_urls ||
    post.payload?.media_urls ||
...
export interface CalendarState {
  ...
  hydrateScheduledPosts: () => Promise<void>;
}
...
export const useCalendarStore = create<CalendarState>((set, get) => ({
  ...
  handleEventUpdate: async (oldYear, oldMonth, oldDay, oldEvent, newYear, newMonth, newDay, newTime) => {
    // Scheduled posts are rescheduled through the schedule API, then re-hydrated from server truth.
    if (oldEvent.scheduled_post_id && newTime) {
      try {
        ...
        const response = await fetch(`/api/schedule/posts/${oldEvent.scheduled_post_id}/reschedule`, {
          method: 'PATCH',
          headers: {
        ...
        const message = error instanceof Error ? error.message : GENERIC_ERRORS.UNKNOWN_ERROR;
        toast.error(CALENDAR_ERRORS.UPDATE_SCHEDULE_FAILED(message));
  ...
  syncCalendarWithPostStatuses: (updates, options) => {
    ...
    const updateMap = updates.reduce<Record<string, CalendarStatusUpdate>>((acc, item) => {
      const normalizedStatus = normalizeLateLifecycleStatus(item.status);
      const postIdStr = String(item.postId || '');
      const syncMeta = item as { _isSyncFromLoad?: boolean };
      acc[postIdStr] = {
        noteType: statusToNoteTypeMap[normalizedStatus],
        status: normalizedStatus,
        url: item.url,
        isSyncFromLoad: syncMeta._isSyncFromLoad === true
      };
      return acc;
    }, {});
    ...
    set((state) => {
      ...
      for (const [dateKey, events] of Object.entries(state.calendarEvents)) {
        ...
        const newEvents = events.map((event) => {
          ...
          const isSyncFromLoad = update.isSyncFromLoad === true;
          if (!isSyncFromLoad) {
  ...
  hydrateScheduledPosts: async () => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(typeof errorData?.error === 'string' ? errorData.error : GENERIC_ERRORS.UNKNOWN_ERROR);
      }

      const result = await response.json();
      const scheduledPosts = Array.isArray(result?.data)
        ? (result.data as ScheduledCalendarPost[])
        : [];

      set((state) => {
        const scheduledEventsByDate = new Map<string, CalendarEvent[]>();

        scheduledPosts.forEach((post) => {
          const mappedPost = mapScheduledPostToCalendarEvent(post);
          if (!mappedPost) return;

          const dayEvents = scheduledEventsByDate.get(mappedPost.dateKey) || [];
          const existingIndex = dayEvents.findIndex(
            (event) => event.scheduled_post_id && String(event.scheduled_post_id) === String(post.id)
          );

          if (existingIndex >= 0) {
            dayEvents[existingIndex] = mappedPost.event;
          } else {
            dayEvents.push(mappedPost.event);
          }

          dayEvents.sort((a, b) => (a.time || '').localeCompare(b.time || ''));
          scheduledEventsByDate.set(mappedPost.dateKey, dayEvents);
        });

        const mergedEvents: Record<string, CalendarEvent[]> = {};
        const allDateKeys = new Set([
          ...Object.keys(state.calendarEvents),
          ...scheduledEventsByDate.keys(),
        ]);

        allDateKeys.forEach((dateKey) => {
          const localEvents = state.calendarEvents[dateKey] || [];
          const preservedLocalEvents = localEvents.filter((event) => {
            if (!event.scheduled_post_id) {
              return true;
            }

            const normalizedStatus = normalizeLateLifecycleStatus(event.status);
            return isFinalLateStatus(normalizedStatus);
          });
          ...
          if (nextEvents.length > 0) {
            mergedEvents[dateKey] = nextEvents;
          }
        });
      ...
      const message = error instanceof Error ? error.message : GENERIC_ERRORS.UNKNOWN_ERROR;
      console.error('[calendar] Failed to hydrate scheduled posts:', message);
    }
  },
}));
