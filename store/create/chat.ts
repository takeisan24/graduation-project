...
${VIDEO_SCRIPT_TEMPLATES}
    ...
    try {
      ...
      const workspaceStore = useCreateWorkspaceStore.getState();
      ...
      const activeContext = activePostId
        ? postsStore.postContextMap[activePostId]
        : postsStore.postContextMap[postsStore.selectedPostId];
      const backendDraftId = activeContext?.draftId && activeContext?.projectId
        ? activeContext.draftId
        : undefined;
      const projectId = activeContext?.projectId || workspaceStore.projectId || undefined;
      const draftId = backendDraftId;
      const chatContext = draftId ? 'draft' : projectId ? 'project' : 'general';
      const contextKey = `${chatContext}:${projectId || ''}:${draftId || ''}`;
      const currentSessionId = get().sessionContextKey === contextKey ? get().sessionId : null;
...
\`\`\`\n`;
      ...
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          platform: 'general',
          sessionId: currentSessionId, // Truyền sessionId để BE biết context
          projectId,
          draftId,
      ...
      if (data?.sessionId) {
        set({ sessionId: data.sessionId, sessionContextKey: contextKey });
      }
