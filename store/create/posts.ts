...
interface CreatePostsState {
  ...
  openPostFromUrl: (platform: string, content?: string, eventMapping?: { eventId: string; dateKey: string }, mediaUrls?: string[], postMedia?: Record<number, MediaFile[]>, onLoadMedia?: (postId: number, media: MediaFile[]) => void) => void;
...
export const useCreatePostsStore = create<CreatePostsState>((set, get) => ({
  ...
  openPostFromUrl: (platform, content = '', eventMapping, mediaUrls, postMedia, onLoadMedia) => {
    ...
    const { openPosts } = get();
    const existing = openPosts.find(p => p.type === platformType);
  ...
  handleTranslatePost: async (postId, targetLanguage) => {
    ...
    try {
      ...
      toast.error(error.message || "Lỗi khi dịch văn bản.");
    } finally {
