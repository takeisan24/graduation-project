...
interface FailedStatusSyncUpdate {
  ...
  status: 'failed';
  url: null;
  _isSyncFromLoad?: boolean;
}

interface FailedApiPostRecord {
  id?: string | number;
...
function mapApiFailedPost(apiPost: FailedApiPostRecord): FailedPost {
  return {
...
function normalizeLocalFailedPost(post: FailedPost): FailedPost {
  return {
    ...post,
    id: String(post.id),
...
export const useFailedPostsStore = create<FailedPostsState>((set, get) => ({
    ...
    try {
      ...
      if (session?.access_token) {
        ...
        if (response.ok) {
          ...
          const convertedPosts: FailedPost[] = apiPosts.map((apiPost: FailedApiPostRecord) => mapApiFailedPost(apiPost));
          ...
          useCalendarStore.getState().syncCalendarWithPostStatuses(
            ...
            } satisfies FailedStatusSyncUpdate)),
      ...
      const localPosts = loadFromLocalStorage<FailedPost[]>('failedPosts', []).map((post) => normalizeLocalFailedPost(post));
      set({
  ...
  loadMoreFailedPosts: async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newPosts: FailedPost[] = apiPosts.map((apiPost: FailedApiPostRecord) => mapApiFailedPost(apiPost));
  ...
  handleDeleteFailedPost: async (id) => {
    ...
    try {
      ...
      const response = await fetch(`/api/schedule/posts/${scheduledPostId}`, {
        method: 'DELETE',
        headers: {
          'authorization': `Bearer ${session.access_token}`
        }
      ...
      toast.error(GENERIC_ERRORS.DELETE_FAILED(message));
      return false;
    }
  },
}));
