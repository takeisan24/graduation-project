                ...
                {!isEditingTime ? (
                    ...
                    <div className="flex items-center justify-center gap-3 px-3" style={{ minHeight: '55px' }}>
                        <button title={t('viewEdit')} onClick={() => onOpenInEditor(event, date)} className="w-10 h-10 p-2 rounded-full hover:bg-white/10 flex-shrink-0 flex items-center justify-center transition-colors">
                            ...
                            <img src="/icons/sidebar/Eye.svg" alt="view" className="opacity-80" style={{ width: 24, height: 24 }} />
                        </button>
                        {isYellowNote && (
                            <>
                                ...
                                <button title={t('editTime')} onClick={handleEditTimeClick} className="w-10 h-10 p-2 rounded-full hover:bg-white/10 flex-shrink-0 flex items-center justify-center transition-colors">
                                    <img src="/icons/sidebar/Clock.svg" alt="edit time" className="opacity-80" style={{ width: 20, height: 20 }}/>
                                </button>
                                <div className="w-px h-6 bg-white/10" />
                                <button title={t('deleteEvent')} onClick={() => onConfirmDelete(event, date)} className="w-10 h-10 p-2 rounded-full hover:bg-white/10 flex-shrink-0 flex items-center justify-center transition-colors">
                                    <img src="/icons/sidebar/Trash.svg" alt="delete" className="opacity-80" style={{ width: 20, height: 20 }} />
                                </button>
                            </>
                        )}
                    </div>
                    ...
                    <div className="p-4 flex flex-col gap-4 w-80">
                        ...
                        <div className="flex items-center justify-center gap-2 text-foreground">
                            <select value={hour} onChange={e => setHour(parseInt(e.target.value))} className="bg-secondary border border-border rounded p-2 text-lg text-foreground">
                                {Array.from({ length: 12 }, (_, i) => i + 1).map(h => <option key={h} value={h}>{String(h).padStart(2, '0')}</option>)}
                            </select>
                            <span>:</span>
                            <select value={minute} onChange={e => setMinute(parseInt(e.target.value))} className="bg-secondary border border-border rounded p-2 text-lg text-foreground">
                                {Array.from({ length: 60 }, (_, i) => i).map(m => <option key={m} value={m}>{String(m).padStart(2, '0')}</option>)}
                            </select>
                            <select value={ampm} onChange={e => setAmpm(e.target.value as 'AM' | 'PM')} className="bg-secondary border border-border rounded p-2 text-lg text-foreground">
