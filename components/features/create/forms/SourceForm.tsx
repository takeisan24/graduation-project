...
import { ArrowLeft, ArrowRight, Plus, X, Link as LinkIcon } from 'lucide-react';
      ...
      <div className={`flex flex-col ${isReadOnly ? "pointer-events-none opacity-90" : ""}`}>
        ...
        <div className="px-4 py-3 border-t border-border bg-secondary/30 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button data-testid="source-form-next-step" onClick={handleNextStep} disabled={!canProceedToStep2} className="px-5 py-2 bg-gradient-to-r from-primary to-primary/80 text-primary-foreground rounded-lg font-medium text-xs disabled:opacity-40 shadow-sm flex items-center gap-1.5">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 pb-4 space-y-5 scrollbar-thin scrollbar-thumb-primary/60 scrollbar-track-secondary/50">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Textarea
              data-testid="source-idea-textarea"
              readOnly={isReadOnly}
          ...
          <div>
            ...
            {isSelectingSource && (
              <div className="grid grid-cols-4 gap-2 animate-in fade-in zoom-in-95 duration-200">
                <button data-testid="source-type-youtube" onClick={() => handleSelectSourceType('youtube')} className="p-3 rounded-lg border border-border bg-secondary hover:border-primary/50 hover:bg-primary/5 flex flex-col items-center gap-1 transition-all">
                  <span className="text-xl">📺</span> <span className="text-[10px] uppercase font-bold text-muted-foreground">YouTube</span>
                </button>
                <button data-testid="source-type-tiktok" onClick={() => handleSelectSourceType('tiktok')} className="p-3 rounded-lg border border-border bg-secondary hover:border-primary/50 hover:bg-primary/5 flex flex-col items-center gap-1 transition-all">
              ...
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                {sourceType === 'youtube' && (
                  <div className="relative">
                    ...
                    <input data-testid="source-youtube-url" type="url" readOnly={isReadOnly} placeholder="https://youtube.com/..." className="w-full bg-background border border-border pl-10 pr-4 py-3 rounded-lg text-sm text-foreground focus:border-primary/50 focus:outline-none" value={youtubeUrl} onChange={(e) => setYoutubeUrl(e.target.value)} autoFocus />
                  </div>
                )}
                {sourceType === 'tiktok' && (
                  <div className="relative">
                    <span className="absolute left-3 top-3 text-lg">🎵</span>
                    <input data-testid="source-tiktok-url" type="url" readOnly={isReadOnly} placeholder="https://tiktok.com/..." className="w-full bg-background border border-border pl-10 pr-4 py-3 rounded-lg text-sm text-foreground focus:border-primary/50 focus:outline-none" value={tiktokUrl} onChange={(e) => setTiktokUrl(e.target.value)} autoFocus />
                  </div>
                )}
                {sourceType === 'article' && (
                  <div className="relative">
                    <span className="absolute left-3 top-3 text-lg">📰</span>
                    <input data-testid="source-article-url" type="url" readOnly={isReadOnly} placeholder="https://example.com/..." className="w-full bg-background border border-border pl-10 pr-4 py-3 rounded-lg text-sm text-foreground focus:border-primary/50 focus:outline-none" value={articleUrl} onChange={(e) => setArticleUrl(e.target.value)} autoFocus />
                  </div>
                )}
                {sourceType === 'file' && (
                  <div className="border-2 border-dashed border-border rounded-lg p-4 hover:bg-secondary transition-colors relative">
                    <input data-testid="source-file-input" type="file" readOnly={isReadOnly} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".pdf,audio/*,.doc,.docx" onChange={(e) => { const f = e.target.files?.[0]; if (f) setSelectedFile(f); }} />
            ...
            {!isReadOnly && !isSelectingSource && sourceType === 'text' && (
              <button data-testid="source-open-attachment-picker" onClick={() => setIsSelectingSource(true)} className="w-full py-3 rounded-lg border border-dashed border-border hover:border-primary/50 hover:bg-primary/5 flex items-center justify-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-all">
      ...
      <div className="px-6 py-4 border-t border-border flex-shrink-0 bg-background">
        ...
        <div className="flex gap-3">
          {isReadOnly ? (
            ...
            <button
              data-testid="source-submit-button"
              onClick={handleAdd}
