...
export default function SourceForm({ onComplete, onCancel, initialData, isReadOnly = false }: SourceFormProps) {
  ...
  if (step === 1) {
    return (
      <div className="h-full flex flex-col bg-[#1E1E23]">
        <div className="px-6 py-4 border-b border-white/10 flex-shrink-0">
          <h2 className="text-lg font-semibold text-white">{tStrategy('title')}</h2>
          <p className="text-sm text-gray-400 mt-1">{tStrategy('subtitle')}</p>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-6 pb-4 scrollbar-thin scrollbar-thumb-primary/60 scrollbar-track-gray-800/50">
        ...
        <div className="px-6 py-4 border-t border-white/10 flex-shrink-0 bg-[#1E1E23]">
          {!canProceedToStep2 ? (
          ...
          <div className="flex gap-3">
            {onCancel && <button onClick={onCancel} className="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-3 rounded-lg font-medium">{t('cancelButton')}</button>}
            <button onClick={handleNextStep} disabled={!canProceedToStep2} className="flex-1 bg-gradient-to-r from-[#E33265] to-[#c52b57] text-white py-3 rounded-lg font-medium disabled:opacity-50 shadow-lg shadow-[#E33265]/30 flex items-center justify-center gap-2">
  ...
  return (
    <div className="h-full flex flex-col bg-[#1E1E23]">
      {/* Header */}
      <div className="px-6 py-4 border-b border-white/10 flex-shrink-0">
        <button onClick={handleBackStep} className="flex items-center gap-2 text-sm text-gray-400 hover:text-white mb-3">
          <ArrowLeft className="w-4 h-4" /> {tStrategy('backButton')}
        </button>
        <h2 className="text-lg font-semibold text-white">{t('title')}</h2>
        <p className="text-sm text-gray-400 mt-1">{tStrategy('step2Subtitle')}</p>
      </div>

      {/* Body */}
      <div className="flex-1 overflow-y-auto px-6 py-6 pb-4 space-y-5 scrollbar-thin scrollbar-thumb-primary/60 scrollbar-track-gray-800/50">
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-white block mb-2">💭 {t('ideaProvided')}</label>
            <Textarea
              readOnly={isReadOnly}
              placeholder={getPlaceholder()}
              className="bg-[#1E1E23] text-white placeholder:text-gray-500 min-h-[150px] resize-none border-white/20 focus:border-primary/50"
              value={textContent}
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {isSelectingSource && (
                <button onClick={() => setIsSelectingSource(false)} className="text-xs text-gray-400 hover:text-white flex items-center gap-1 bg-white/5 px-2 py-1 rounded">
                  <X className="w-3 h-3" /> {t('cancelButton')}
                </button>
              )}
            </div>
            ...
            {isSelectingSource && (
              <div className="grid grid-cols-4 gap-2 animate-in fade-in zoom-in-95 duration-200">
                <button onClick={() => handleSelectSourceType('youtube')} className="p-3 rounded-lg border border-white/20 bg-[#25252b] hover:border-primary/50 hover:bg-primary/5 flex flex-col items-center gap-1 transition-all">
                  <span className="text-xl">📺</span> <span className="text-[10px] uppercase font-bold text-gray-400">YouTube</span>
                </button>
                <button onClick={() => handleSelectSourceType('tiktok')} className="p-3 rounded-lg border border-white/20 bg-[#25252b] hover:border-primary/50 hover:bg-primary/5 flex flex-col items-center gap-1 transition-all">
                  <span className="text-xl">🎵</span> <span className="text-[10px] uppercase font-bold text-gray-400">TikTok</span>
                </button>
                <button onClick={() => handleSelectSourceType('article')} className="p-3 rounded-lg border border-white/20 bg-[#25252b] hover:border-primary/50 hover:bg-primary/5 flex flex-col items-center gap-1 transition-all">
                  <span className="text-xl">📰</span> <span className="text-[10px] uppercase font-bold text-gray-400">Artile</span>
                </button>
                <button onClick={() => handleSelectSourceType('file')} className="p-3 rounded-lg border border-white/20 bg-[#25252b] hover:border-primary/50 hover:bg-primary/5 flex flex-col items-center gap-1 transition-all">
                  <span className="text-xl">📁</span> <span className="text-[10px] uppercase font-bold text-gray-400">File</span>
                </button>
              </div>
            )}
              ...
              <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                {sourceType === 'youtube' && (
                  <div className="relative">
                    ...
                    <input type="url" readOnly={isReadOnly} placeholder="https://youtube.com/..." className="w-full bg-[#1E1E23] border border-white/20 pl-10 pr-4 py-3 rounded-lg text-sm text-white focus:border-primary/50 focus:outline-none" value={youtubeUrl} onChange={(e) => setYoutubeUrl(e.target.value)} autoFocus />
                  </div>
                )}
                {sourceType === 'tiktok' && (
                  <div className="relative">
                    <span className="absolute left-3 top-3 text-lg">🎵</span>
                    <input type="url" readOnly={isReadOnly} placeholder="https://tiktok.com/..." className="w-full bg-[#1E1E23] border border-white/20 pl-10 pr-4 py-3 rounded-lg text-sm text-white focus:border-primary/50 focus:outline-none" value={tiktokUrl} onChange={(e) => setTiktokUrl(e.target.value)} autoFocus />
                  </div>
                )}
                {sourceType === 'article' && (
                  <div className="relative">
                    <span className="absolute left-3 top-3 text-lg">📰</span>
                    <input type="url" readOnly={isReadOnly} placeholder="https://example.com/..." className="w-full bg-[#1E1E23] border border-white/20 pl-10 pr-4 py-3 rounded-lg text-sm text-white focus:border-primary/50 focus:outline-none" value={articleUrl} onChange={(e) => setArticleUrl(e.target.value)} autoFocus />
                  </div>
                )}
                {sourceType === 'file' && (
                  <div className="border-2 border-dashed border-white/20 rounded-lg p-4 hover:bg-white/5 transition-colors relative">
                    ...
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <div className="text-sm">
                        {selectedFile ? <span className="text-green-400 font-medium">{selectedFile.name}</span> : <span className="text-gray-400">Click hoặc kéo thả file vào đây</span>}
                      </div>
                    </div>
                  </div>
                )}
            ...
            {!isReadOnly && !isSelectingSource && sourceType === 'text' && (
              <button onClick={() => setIsSelectingSource(true)} className="w-full py-3 rounded-lg border border-dashed border-white/20 hover:border-primary/50 hover:bg-primary/5 flex items-center justify-center gap-2 text-sm text-gray-400 hover:text-white transition-all">
      ...
      <div className="px-6 py-4 border-t border-white/10 flex-shrink-0 bg-[#1E1E23]">
        ...
        <div className="flex gap-3">
          {isReadOnly ? (
            ...
            <button
              onClick={handleAdd}
              className="flex-1 bg-gradient-to-r from-[#E33265] to-[#c52b57] text-white py-3 rounded-lg font-medium shadow-lg shadow-[#E33265]/30 disabled:opacity-50"
