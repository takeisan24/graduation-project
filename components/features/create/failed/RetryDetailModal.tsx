...
export function RetryDetailModal({ post, onClose, onConfirmReschedule, onEdit, isPreview = false }: RetryDetailModalProps) {
  const t = useTranslations("CreatePage.failedSection.retryModal");
  ...
  const account = post.profileName && post.profilePic
    ...
    : { username: post.platform, profilePic: "" };
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-2xl w-[600px] max-w-[95vw] shadow-xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 overflow-y-auto">
          ...
          <div className="mb-5">
            <div className="text-foreground/80 text-sm font-semibold mb-2">{t("postContent")}</div>
            <div className="bg-muted border border-border rounded-lg p-4 text-foreground whitespace-pre-wrap max-h-[200px] overflow-y-auto">{post.content}</div>
          </div>
          <div className={`border rounded-xl p-5 mb-5 ${recoveryAccent}`}>
            ...
            {post.errorMessage && (
              <div className="mt-3 text-sm text-foreground/80 leading-6">
                <span className="font-semibold text-foreground">{t("platformDetails")}</span> {post.errorMessage}
              </div>
            )}
          </div>
          {isContentIssue ? (
            <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => onEdit(post)}>{t("editPost")}</Button>
          ) : (
              ...
              {canAutoReschedule ? (
                <>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div className="text-foreground font-semibold">{t("rescheduleTime")}</div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-muted-foreground text-sm mb-2">{t("dateLabel")}</label>
                      <Input type="date" value={rescheduleDate} onChange={(e) => setRescheduleDate(e.target.value)} />
                    </div>
                    <div>
                      <label className="block text-muted-foreground text-sm mb-2">{t("timeLabel")}</label>
        ...
        {!isContentIssue && (
          <div className="px-6 pb-6 flex items-center justify-between gap-4 border-t border-border pt-4 mt-auto">
            <div className="flex items-center gap-3">
              <Button className="bg-blue-600 hover:bg-blue-700" onClick={() => onEdit(post)}>{t("editBtn")}</Button>
            </div>
            <div className="flex items-center gap-4">
              <Button variant="outline" className="border-border hover:bg-secondary" onClick={onClose}>{t("cancel")}</Button>
              {canAutoReschedule ? (
                <Button className="bg-primary hover:bg-primary/90" onClick={handleConfirmClick}>{t("scheduleAgain")}</Button>
              ) : null}
            </div>
          </div>
        )}
