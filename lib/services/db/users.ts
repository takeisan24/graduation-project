...
export async function getUserProfile(userId: string): Promise<UserProfile | null> {
  ...
  if (error && error.code !== "PGRST116") {
    if (isMissingSchemaResource(error)) {
      console.warn("[db/users] Billing columns missing on users table; falling back to free plan.");
      ...
      if (fallbackError && fallbackError.code !== "PGRST116") {
        console.warn("[db/users] Error fetching fallback user profile:", fallbackError);
        return null;
      }
  ...
  return {
    ...
    subscription_ends_at: typeof data.subscription_ends_at === "string" ? data.subscription_ends_at : null,
    next_credit_grant_at: typeof data.next_credit_grant_at === "string" ? data.next_credit_grant_at : null,
  } as UserProfile;
}
...
export async function getUserPlanAndCredits(userId: string): Promise<{ plan: string; credits_balance: number; subscription_ends_at?: string | null; next_credit_grant_at?: string | null } | null> {
  ...
  return {
    ...
    next_credit_grant_at: profile.next_credit_grant_at ?? null
  };
}
