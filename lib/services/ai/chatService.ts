...
import { createChatSession } from "@/lib/services/db/chatSessions";
import {
  ...
  try {
    ...
    while ((match = jsonBlockPattern.exec(reply)) !== null) {
      ...
      try {
        ...
        let parsedResponse;
        try {
          parsedResponse = JSON.parse(escapedJson);
        } catch {
...
export async function processChatMessage(
  ...
  try {
    response = await askAssistant({
      ...
      platform: platform,
      context: draftId ? 'draft' : projectId ? 'project' : (context as 'general' | 'project' | 'draft' | 'workspace'),
