...
export async function generateFromSourceWithCredits(
  ...
  const documentFilesToUpload: { uri: string; mimeType: string }[] = [];
        ...
        if (isTokenLimitError) {
          ...
          try {
            ...
            if (metadata) {
              ...
              const _instructions = promptParts
...
Chỉ trả về văn bản.`;
      ...
      const combinedPrompt = `${promptParts.filter((part) => typeof part === 'string').join('\n\n')}\n\nNội dung audio đã được trích xuất:\n\n${extractedContent}`;
      if (!combinedPrompt || combinedPrompt.trim().length === 0) {
