...
function isMissingSchemaResource(error: { code?: string } | null | undefined): boolean {
  return error?.code === "42703" || error?.code === "PGRST205";
}

async function getUserBillingState(userId: string): Promise<{
  plan: string;
  ...
  if (error) {
    if (!isMissingSchemaResource(error) && error.code !== "PGRST116") {
      console.warn("[usage] Error fetching user billing state:", error);
    }
    return { plan: "free", subscription_status: null, credits_balance: 0 };
  }

  return {
    plan: typeof data?.plan === "string" ? data.plan : "free",
    subscription_status: typeof data?.subscription_status === "string" ? data.subscription_status : null,
    credits_balance: typeof data?.credits_balance === "number" ? data.credits_balance : 0,
  };
}
...
export async function ensureUsageRow(userId: string, timeZone: string = DEFAULT_TIMEZONE) {
  ...
  if (selectError && isMissingSchemaResource(selectError)) return null;
  if (data) return data;
  ...
  if (error) {
    if (isMissingSchemaResource(error)) return null;
    throw error;
  }
  return insertData;
}
...
export async function checkCredits(userId: string, action: keyof typeof CREDIT_COSTS): Promise<{
  ...
  if (usageError && usageError.code !== 'PGRST116' && !isMissingSchemaResource(usageError)) {
  ...
  if (!balanceError && userWithBalance && userWithBalance.credits_balance !== undefined && userWithBalance.credits_balance !== null) {
    ...
    if (balanceError && !isMissingSchemaResource(balanceError)) {
    ...
    if (updateError && !isMissingSchemaResource(updateError)) {
...
export async function addPurchasedCredits(userId: string, amount: number): Promise<{
  ...
  if (error) {
    if (isMissingSchemaResource(error)) return { success: true };
...
export async function trackUsage(
  ...
  if (error) {
    if (isMissingSchemaResource(error)) return;
...
export async function checkPostLimit(userId: string): Promise<{
  ...
  if (error && isMissingSchemaResource(error)) {
    return { canSchedule: true, current: 0, limit };
  }
